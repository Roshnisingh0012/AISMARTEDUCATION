...
import { fetchDashboardSummary, enrollCourse } from '../api/learner';
import { DashboardSummary } from '../types';
import { AlertCircle, Target, BookOpen, CheckCircle, TrendingDown, TrendingUp, BarChart3 } from 'lucide-react';
import {
  Radar,
  RadarChart,
  PolarGrid,
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center">
            <BarChart3 className="mr-2 h-5 w-5 text-indigo-600" /> Competency & Skill Gap Breakdown
          </h2>
          <div className="space-y-4">
            {summary.completed_assessments_count > 0 ? (
              (summary.skill_gaps || []).map((gap) => {
                ...
                let statusBadge = <span className="text-xs font-bold px-2 py-1 bg-green-100 text-green-700 rounded-full">🟢 Proficient</span>;
                if (!isProficient) {
                  if (isCritical) {
                    statusBadge = <span className="text-xs font-bold px-2 py-1 bg-red-100 text-red-700 rounded-full">🔴 Critical Gap</span>;
                  } else {
                    statusBadge = <span className="text-xs font-bold px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full">🟡 Moderate Gap</span>;
                  }
                }
                ...
                return (
                  <div key={gap.competency_id} className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h3 className="font-semibold text-gray-900">{gap.competency_name}</h3>
                        <p className="text-xs text-gray-500 mt-0.5">Current: {gap.current_score}% / Target: {gap.benchmark_score}%</p>
                      </div>
                      {statusBadge}
                    </div>
                    ...
                    <div className="relative h-2 w-full bg-gray-200 rounded-full overflow-hidden mt-3">
                      <div 
                        className={`absolute top-0 left-0 h-full ${isProficient ? 'bg-green-500' : isCritical ? 'bg-red-500' : 'bg-yellow-500'}`} 
                        style={{ width: `${Math.min(100, Math.max(0, gap.current_score))}%` }}
                      />
                      <div 
                    ...
                    <div className="flex justify-between items-center mt-2 text-xs">
                      ...
                      <span className={`font-semibold flex items-center ${isProficient ? 'text-green-600' : 'text-red-600'}`}>
                        {isProficient ? <TrendingUp className="h-3 w-3 mr-1" /> : <TrendingDown className="h-3 w-3 mr-1" />}
                        {deficit > 0 ? '+' : ''}{deficit.toFixed(1)}% {isProficient ? 'Surplus' : 'Deficit'}
                      </span>
              ...
              <div className="p-4 text-center text-gray-500">Take an assessment to generate your competency data.</div>
            )}
          </div>
        </div>
            ...
            {summary.completed_assessments_count > 0 && (summary.skill_gaps || []).length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={summary.skill_gaps.map(g => ({
                  ...
                  <Radar name="Target Benchmark" dataKey="Target" stroke="#818cf8" fill="#c7d2fe" fillOpacity={0.3} />
                  <Radar name="Your Score" dataKey="Actual" stroke="#10b981" fill="#34d399" fillOpacity={0.5} />
              ...
              <div className="h-full flex items-center justify-center text-gray-500">
                Take an assessment to generate your competency data and radar chart.
              </div>
            )}
          </div>
        </div>
      ...
      <div>
        <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center">
          <BookOpen className="mr-2" /> {t('dashboard.topCourses')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {summary.recent_recommendations.map(course => (
            <div key={course.course_id} className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition">
              ...
              <button 
                onClick={async (e) => {
                  e.currentTarget.innerText = 'Enrolling...';
                  try {
                    const res = await enrollCourse(course.course_id);
                    if(res.external_url) {
                      window.open(res.external_url, '_blank');
                    }
                    alert(res.message);
                    e.currentTarget.innerText = 'Enrolled / In Progress';
                    e.currentTarget.className = "mt-5 w-full bg-green-600 text-white font-medium py-2 rounded-lg";
                    e.currentTarget.disabled = true;
                  } catch (err) {
                    alert('Enrollment failed');
                    e.currentTarget.innerText = 'Enroll Now';
                  }
                }}
