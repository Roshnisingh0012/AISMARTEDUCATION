    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl max-w-2xl w-full p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 mt-4">
          ...
          <h2 className="text-3xl font-bold text-gray-900">{t('quiz_runner.assessmentComplete', 'Assessment Complete!')}</h2>
          <p className="text-gray-500 mt-2">{t('quiz_runner.proficiencyEvaluated', 'Your proficiency has been evaluated and recorded.')}</p>
        </div>

        <div className="flex justify-center mb-8">
          <div className="text-center bg-gray-50 p-6 rounded-xl border border-gray-100 min-w-[200px]">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">{t('quiz_runner.finalScore', 'Final Score')}</span>
          ...
          {result.results.map((res, i) => (
            <div key={res.question_id} className={`p-4 rounded-lg border ${res.is_correct ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className="flex items-start">
                ...
                <div>
                  <p className="font-medium text-gray-900">{t('quiz_runner.question')} {i + 1}</p>
                  {!res.is_correct && (res.explanation || res.explanation_hi) && (
                    <p className="text-sm text-gray-600 mt-2 bg-white p-2 rounded border border-red-100 shadow-sm">
                      <strong className="text-gray-800">{t('quiz_runner.explanation', 'Explanation:')} </strong>
                      {isHi && res.explanation_hi ? res.explanation_hi : res.explanation}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          >
            {t('quiz_runner.closeResults', 'Close Results')}
          </button>
          <button 
          ...
          >
            {t('quiz_runner.viewPathway', 'View Learning Pathway')} <ArrowRight size={18} className="ml-2" />
          </button>
        </div>
      </div>
    </div>
